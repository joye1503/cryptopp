...
const ECP::Point& ECP::Double(const Point &P) const
{
	...
	{
		if (P.IsIdentity() || P.y==GetField().Identity()) return Identity();
...
const ECP::Point& ECP::NistAdd(const Point &P, const Point &Q) const
{
	if (P.IsIdentity()) return Q;
	if (Q.IsIdentity()) return P;
	if (GetField().Equal(P.x, Q.x))
...
// Optimized for NIST curves when a = -3
const ECP::Point& ECP::NistDouble(const Point &P) const
{
	if (P.IsIdentity() || P.y==GetField().Identity()) return Identity();
